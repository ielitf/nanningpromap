...
//            heartBeatMsg = makeHeartBeatMsg();
    ...
    private byte[] makeHeartBeatMsg() {
        ...
        builder_CHearBeat.setType(devInfo.devType);
        builder_CHearBeat.setIdentify(devInfo.identify);
        //可用磁盘空间单位为：MB
    ...
    //intervalTime：单位：毫秒
    public HeartBeatThread(IoSession session, DeviceInfo deviceInfo, int intervalTime) {
        //Log.d(TAG, "start");
        try {
